...
# Initialize session state for room booking status
...
def book_rooms(num_rooms):
    """Book the optimal rooms based on priority queue."""
    available_rooms = [(get_room_number(i), i) for i, booked in enumerate(st.session_state.room_status) if not booked]
    heapq.heapify(available_rooms)
    
    if len(available_rooms) < num_rooms:
        return []
    
    booked = [heapq.heappop(available_rooms)[1] for _ in range(num_rooms)]
    for i in booked:
